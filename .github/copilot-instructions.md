...
- If a `work/YYYY_MM_DD_<branch-name>/PLAN.md` exists, read it to understand the approved plan: the goal, the task list, and any stated constraints or design decisions.
...
- Comment explicitly on any point where the agent was confused, was unable to use a tool (or a tool failed), or did not follow its instructions — these are the most valuable signals for improving future planning and agent instructions.
...
Review the implementation changes independently of the agent process review.
