...
If the PR includes files under `work/<branch-name>/`, review how well the agent executed the plan.

- Read `work/<branch-name>/PLAN.md` to understand the approved plan: the goal, the task list, and any stated constraints or design decisions.
- Read each `work/<branch-name>/<session-id>.md` file. These are distilled records of the agent's actual behaviour during the work session.
