        ...
        <span className="ml-auto flex items-center gap-3">
          ...
          {app.user ? (
            <span className="flex items-center gap-2">
              <img src={app.user.avatarUrl} alt={app.user.name} className="w-8 h-8 rounded-full" />
              <button
                onClick={() => app.signOut()}
                className="text-sm text-navy/60 hover:text-navy cursor-pointer"
              >
                Sign out
              </button>
            </span>
            ...
            <form method="post" action="/auth/test-login">
              <button
                type="submit"
                className="text-sm bg-navy text-white px-3 py-2 rounded-md hover:bg-navy/80 cursor-pointer"
              >
            ...
            <a
              href="/auth/github"
              className="text-sm bg-crimson text-white px-3 py-2 rounded-md hover:bg-terracotta"
            >
              Sign in
            </a>
          )}
