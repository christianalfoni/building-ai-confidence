...
src/
  ...
  html-template.gen.ts        # Generated by build — the built index.html as a string (gitignored; placeholder committed)
...
1. **Type-check** — `tsc -b` (emit is disabled; this is a lint step only)
2. **Client** — `vite build --outDir dist/client` — produces `dist/client/index.html` (with hashed CSS/JS asset references) and `dist/client/assets/`
3. **Embed template** — `node scripts/gen-html-template.mjs` — reads `dist/client/index.html` and writes it as a TypeScript string export to `src/html-template.gen.ts`
...
`api/index.js` is **not committed to git** — it is generated by the build and picked up by Vercel after the build runs. `src/html-template.gen.ts` is also gitignored (a placeholder with an empty string is committed so TypeScript can resolve the import during type-checking).
